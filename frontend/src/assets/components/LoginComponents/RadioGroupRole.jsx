...
const RadioGroupRole = ({ label, options, ...props }) => {
    ...
    return (
        <div role="group" aria-labelledby={`${props.name}-group`} className="flex flex-col mb-2">
            ...
            <div className="flex items-center justify-between"> {/* Horizontal alignment */}
                {options.map((option) => (
                    <label key={option.value} className={`flex items-center justify-end ${field.value === option.value ? 'text-blue-500' : ''}`} >
                        <input
                            type="radio"
                            {...field}
                            value={option.value}
                        ...
                        />
                        <span className="ml-3 text-sm">{option.label}</span> {/* Option text */}
                    </label>
                ))}
            </div>
